...
export async function getTrendingSnapshot(rawTicker: string): Promise<TrendingItem> {
  ...
  try {
    const [price, stocktwits] = await Promise.all([getPriceHistory(ticker), getStockTwits(ticker)])
...
export async function getTrending(): Promise<TrendingItem[]> {
  ...
  const items = await Promise.all(TRENDING_TICKERS.map(getTrendingSnapshot))
